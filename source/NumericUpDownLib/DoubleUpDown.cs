...
public partial class DoubleUpDown : AbstractBaseUpDown<double>
{
    ...
    {
        get => (double)GetValue(LargeStepSizeProperty);
    ...
    {
        if (double.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out double number))
        {
            tempValue = number;
    ...
    {
        return double.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out number);
    }
