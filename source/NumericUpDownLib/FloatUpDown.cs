...
public partial class FloatUpDown : AbstractBaseUpDown<float>
{
    ...
    {
        get => (float)GetValue(LargeStepSizeProperty);
    ...
    {
        if (float.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out float number))
        {
            tempValue = number;
    ...
    {
        return float.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out number);
    }
