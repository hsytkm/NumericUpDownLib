...
public partial class UIntegerUpDown : AbstractBaseUpDown<uint>
{
    ...
    {
        get => (uint)GetValue(StepSizeProperty);
    ...
    {
        get => (uint)GetValue(LargeStepSizeProperty);
    ...
    {
        if (uint.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out uint number))
        {
            tempValue = number;
    ...
    {
        return uint.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out number);
    }
