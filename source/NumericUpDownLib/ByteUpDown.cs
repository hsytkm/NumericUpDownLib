...
public partial class ByteUpDown : AbstractBaseUpDown<byte>
{
    ...
    {
        get => (byte)GetValue(StepSizeProperty);
    ...
    {
        get => (byte)GetValue(LargeStepSizeProperty);
    ...
    {
        if (byte.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out byte number))
        {
            tempValue = number;
    ...
    {
        return byte.TryParse(text, NumberStyle, CultureInfo.CurrentCulture, out number);
    }
